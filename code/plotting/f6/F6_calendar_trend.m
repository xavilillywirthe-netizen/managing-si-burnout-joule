% Plot calendar-aging lifetime trends for Figure 6.
%
% The master table contains the lifetime diagnostic outputs with experimental
% group labels. This script compares calendar-aging groups by storage
% temperature at 100% SoC using individual-cell trajectories and group averages.
...
% Exclude rows that were not assigned to one of the plotted calendar groups.
if ismember('calendar_group', Master.Properties.VariableNames)
    Master = Master(~strcmp(string(Master.calendar_group), "unassigned"), :);
end

% Calendar-temperature comparison used in the main Figure 6 panel.
compare_group_names = {
 'Cal_100SOC_25C'
 'Cal_100SOC_45C'
 'Cal_100SOC_60C'
...
for i = 1:numel(param_names_main)
    ...
    for g = 1:numel(compare_group_names)
        ...
        [x_mean, y_mean] = compute_group_average_curve(DataG, x_col, pname, intervals(g));
    ...
    xlabel('Calendar Day [-]', 'FontSize', 18);
    set(gca, 'FontSize', 18);
...
print(fig, fullfile(figure_dir, 'F6_calendar_trend.svg'), '-dsvg', '-vector');
